...
namespace Umoxi
{
    public partial class usSalary : UserControl
    {
        public usSalary()
        {
            InitializeComponent();
        ...
        #region instance
        private static usSalary _instance;

        public static usSalary Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new usSalary();
                return _instance;
            }
        }
        #endregion
        ...
        private void btnFindData_Click(object sender, EventArgs e)
        {
            ...
            " WHERE        (EmployeeLedger.PostingType = 'SalaryEntry') AND (EmployeeLedger.EntryDate >= '" + Strings.Format(dtpFrom.DateTime, "MM/dd/yyyy") + "' AND   EmployeeLedger.EntryDate <= '" + Strings.Format(dtpTo.DateTime, "MM/dd/yyyy") + "') ", DataGridView2);
        }
