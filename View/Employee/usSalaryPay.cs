...
namespace Umoxi
{
    public partial class usSalaryPay : UserControl
    {
        public usSalaryPay()
        {
            InitializeComponent();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(cmbSchool.Text))
                ...
                Snackbar.Show(FrmMain.Default, "Required: Month", BunifuSnackbar.MessageTypes.Success);
                    ...
                    foreach (DataGridViewRow row in DataGridView1.Rows)
                    {
                        ...
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ConnectionNode.ExecuteSQLQuery(" INSERT INTO    EmployeeLedger (EmployeeID, User_ID, EntryDate, Month, PostingType, Debit, Credit, Narration) VALUES " +
                                " (" + Convert.ToString(EmployeeID) + ", " + Convert.ToString(ConnectionNode.xUser_ID) + ", '" + Strings.Format(dtpEntryDate.DateTime, "MM/dd/yyyy") + ("', '" + cmbMonth.Text + "', 'SalaryPay', '" + UtilitiesFunctions.str_repl(txtAmount.Text) + "', 0, '" + UtilitiesFunctions.str_repl(txtSalaryPay.Text) + "') "));
                        }
                    }
                    UtilitiesFunctions.Audit_Trail(ConnectionNode.xUser_ID, DateTime.Now.ToLongTimeString(), "Salary payment # " + txtSalaryPay.Text);
                    LoadData();
        ...
        private void btnFindData_Click(object sender, EventArgs e)
        {
            ...
            " WHERE        (EmployeeLedger.PostingType = 'SalaryPay') AND (EmployeeLedger.EntryDate >= '" + Strings.Format(dtpFrom.DateTime, "MM/dd/yyyy") + "' AND   EmployeeLedger.EntryDate <= '" + Strings.Format(dtpTo.DateTime, "MM/dd/yyyy") + "') ", DataGridView2);
